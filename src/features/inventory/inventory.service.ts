...
export interface InventoryTransaction extends Omit<Inventory, 'records'> {
  records: Array<{
    id: string
    item_id: string
    qty_change: number
...
export async function fetchStockLevels(): Promise<InventoryItem[]> {
  const { data, error } = await supabase
    .from('inventory_item')
    .select('*, item_packaging(packaging_id, packaging:packaging(id, pack_eng, pack_arab)), stock:item_stock(id, packaging_id, quantity, avg_cost, packaging:packaging(pack_eng, pack_arab))')
    .is('deleted_at', null)
    .order('item_name')
  assertNoError(error)
...
export async function fetchInventoryTransactions(): Promise<InventoryTransaction[]> {
  const { data, error } = await supabase
    .from('inventory')
    .select('*, records:inventory_record(*, item:inventory_item(item_name))')
    .is('deleted_at', null)
